...
- **Session summary** — podium (with played count per player), full leaderboard from rank #4 down, and round history. For rated sessions, each player's rating and its change from that session are shown too — on the live summary, and retroactively on every past session. Opening a past session from Session History shows the same summary, plus its own **Round history** — collapsed behind a dropdown to stay compact, read-only since those scores already fed into ratings, and left out of the exported/shared image just like the live version's. Add a background photo with a gradient fade effect, reposition it with a slider, and export or share as an image.
...
- **Leaderboard & ratings** — a 4th menu option with a running Elo rating per player (starting at 1000, the same system chess uses), toggled between **This month** (resets everyone to 1000 on the 1st, so a bad month doesn't follow you around) and **All-time** (never resets, for a number that means something over the long run). Ranked by rating rather than raw win rate, since rating accounts for who you played with and against — beating a strong opponent is worth more than beating a weak one, which Win % can't tell apart. Needs 10+ matches in the active scope to be ranked; fewer than that and a player still shows, just below the ranked table with a **provisional** badge, so a small unrepresentative sample can't top it. Tap any name for a detail screen: current rating with both this-month and all-time rank chips, head-to-head record against every opponent (compact 2-column grid), **Best Pair / Worst Pair** — the doubles partners you win most and least often with, side by side — and full match history. A small **Explanation on ratings** link opens a plain-language walkthrough of the calculation.
...
**Leaderboard rename & polish**
- "Monthly standings" is now **Leaderboard** throughout (menu tile, header, phase badge) — reflects that it covers both the monthly and all-time views, not just the monthly one.
- The long mode-explanation text that used to sit on the main screen now lives inside the **Explanation on ratings** popup instead, condensed without losing anything, so the main screen stays focused on the standings themselves.
- All-time view: the "N months · N players" subtitle is now centered under the **All-time Leaderboard** title, instead of left-aligned with nothing balancing it on the right (the monthly view has a ‹ › picker there; all-time doesn't).
- "All-time leaderboard" → **All-time Leaderboard**, capitalised to match "This month" styling elsewhere.
- Player detail screen now shows **both** rank chips — this month's and all-time's — so switching scope doesn't hide where you stand in the other one.

**Best Pair / Worst Pair**
- New section on the player detail screen, right below "Record against each opponent": two columns, **Best Pair** and **Worst Pair**, each the top 3 doubles partners by win rate together. Worst Pair shows the same win% as Best Pair, just sorted the other way — not a separate loss% figure — so a number always means the same thing in both columns and "0%" reads as unambiguously bad instead of a confusing "100%".
- Ranked by win% first, then by how many matches they've actually played together, so a 100% record over 4 matches outranks a 100% record over 1. Singles matches don't contribute, since there's no partner to credit.

**Round history in Session History**
- Opening a past session now also shows its round-by-round history, same as the live Summary screen — but collapsed behind a **Round history** dropdown by default to keep the screen compact, and deliberately read-only (unlike the live version's editable one), since a finished session's scores have already fed into everyone's rating.
- Lives outside the exported card, so it never appears in the downloaded or shared image regardless of whether it's expanded when you tap the button — verified by checking the exported canvas is sized to the summary card alone even while the history list is open on screen.

**Bug fixes**
- Session summary rows for rank 4 and below (both the live Summary and the Session History viewer) had ragged, inconsistently-aligned columns, and long names could wrap the "played" count onto two lines. Fixed with fixed-width columns: played count centered, percentage centered, rating right-aligned. Verified against a real session screenshot and stress-tested with an extreme long name to confirm nothing else shifts.
- Exporting or sharing a summary card with a background photo showed a stray light border at the top, and a faint sliver at the very bottom edge. Both traced back to the card's own light frame — the capture wrapper's own padding and background, and html2canvas's own canvas-fill colour — showing through where the photo card's rounded corners and a sub-pixel rounding gap didn't quite cover it. Both now switch to a dark fill whenever a photo is active. Verified by sampling the exported canvas's actual pixels at every edge and corner.
- The Share button could get stuck on "Preparing…" forever if the native share sheet was cancelled (e.g. after repositioning a photo, then sharing). Root cause: the button-reset code sat inside an un-awaited callback, so a cancelled share never reached it. Fixed by awaiting the whole capture-and-share flow, with the reset guaranteed to run whether it succeeds, fails, or is cancelled. Verified by simulating a cancelled share, a cancelled fallback share, a successful share, and a failed canvas capture — all four correctly reset the button.
